...
scp -i ${KEY_PEM_DIR} -r /home/hwang/My_Code/distributed_experiments ubuntu@${PUB_IP_ADDR}:~/
ssh -i ${KEY_PEM_DIR} ubuntu@${PUB_IP_ADDR} 'sudo apt-get update; cp hosts distributed_experiments; cp distributed_experiments/tools/remote_script.sh ~/'
